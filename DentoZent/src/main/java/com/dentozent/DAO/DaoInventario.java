...
public class DaoInventario {
    ...
    public boolean registrar(InventarioModelo inv, int idUsuario) {
        String sql = "INSERT INTO inventario (nombre_producto, cantidad, fecha_vencimiento, estado, id_usuario) VALUES (?, ?, ?, ?, ?)";

        try (Connection con = new com.dentozent.conexion.Conection().getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {

            ps.setString(1, inv.getNombreProducto());
            ps.setInt(2, inv.getCantidad());
            ps.setDate(3, inv.getFechaVencimiento());
